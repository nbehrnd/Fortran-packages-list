...
# Get the name of the principal branch of a remote repository
repo_owner = "nbehrnd"  # 'owner_username'
repo_name = "Fortran-packages-list"  # 'repository_name'
repo = g.get_repo(f"{repo_owner}/{repo_name}")
principal_branch = repo.default_branch
print(f"The principal branch of {repo_name} is: {principal_branch}")

# Check if the principal branch has a README file
branch = repo.get_branch(principal_branch)
tree = repo.get_git_tree(sha=branch.commit.sha, recursive=True)
readme_exists = any(entry.path == 'README.md' for entry in tree.tree)
if readme_exists:
    print("README.md exists in the principal branch.")
else:
    print("README.md does not exist in the principal branch.")
